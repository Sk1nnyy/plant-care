        ...
        when (event) {
            ...
            is UiEvent.OnPlantTypePicked -> {
                viewModelScope.launch(Dispatchers.IO) {
                    ...
                    uiState.emit(uiState.value.copy(species = speciesDetail))
                }
            }

            is UiEvent.OnCreateClick -> {
                viewModelScope.launch(Dispatchers.IO) {
                    val species = uiState.value.species ?: return@launch
                    val wateringSchedule =
                        when (event.scheduleType) {
                            ...
                            WateringScheduleType.MultipleDaysInWeek ->
                                WateringSchedule.MultipleDaysInWeek(
                                    event.scheduleDays,
                                )
                        }
                    val plant =
