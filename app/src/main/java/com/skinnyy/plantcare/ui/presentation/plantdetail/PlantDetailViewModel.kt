...
class PlantDetailViewModel(
    ...
    init {
        viewModelScope.launch(Dispatchers.IO) {
            ...
            uiState.emit(uiState.value.copy(isLoading = false, species = result.data))
        }
        viewModelScope.launch {
            val id = id.toInt()
            favoritePlantRepository.getById(id).collect {
                uiState.emit(uiState.value.copy(favorite = it != null))
            }
        }
    }
    ...
    private fun addFavorite() {
        val species = uiState.value.species ?: return
        viewModelScope.launch {
